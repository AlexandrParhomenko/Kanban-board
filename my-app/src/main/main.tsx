...
const Main = () => {
  ...
  const [tables, setTables] = useState([
    {id: 1, name: 'TODO', cards: []},
    {
      id: 2, name: 'In progress', cards: [
        {name: 'sad', description: 'asd', comments: ['sd', 'das'], author: 'asd'},
      ]
    },
  ...
  const cardEdit1 = (id: number, cardId: number) => {
    const dsa = tables[id - 1].cards.map((card, idx) => idx === cardId ? {...card, name: name} : card)
    setTables(tables.map(tab => tab.id === id ? {...tab, cards: dsa} : tab))
    setCardEdit(0)
  }
  ...
  const cardEditHandler = (cardName: string, id: number, cardId: number) => {
    return (
        <div className='add-wrapper'>
          <input type='text' autoFocus className='add-input'
  ...
  const descriptionEditHandle = (id: number, cardId: number) => {
    const dsa = tables[id - 1].cards.map((card, idx) => idx === cardId ? {...card, description: description} : card)
    setTables(tables.map(tab => tab.id === id ? {...tab, cards: dsa} : tab))
    setDescription('')
  ...
  const addHandler = (id: number) => {
    ...
    return (
        <div className='add-wrapper'>
          <input type='text' autoFocus className='add-input' placeholder='Enter a title for the card'
                    onChange={(e) => {
                      setName(e.target.value)
                    }}/>
          <div style={{display: 'flex', alignItems: 'center'}}>
                    ...
                    <div>
                      {visible[0] === id + 1 && visible[1] === idx + 1
                          ? <div className='card-popup'>
                            <button className='close-button btn' onClick={() => setVisible([0])}>X
                            </button>
                            <div className='card-content'>
                              ...
                              <div className='description-wrapper'>
                                ...
                                {el.description.length > 1 && descriptionEdit === 0
                                    ...
                                    : <div className='description-wrapper'>
                                      <input defaultValue={el.description} type='text' className='add-input' placeholder='Enter a description'
                                                  onChange={(e) => {
                                                    setDescription(e.target.value)
                                                  }}/>
                                      <button onClick={() => descriptionEditHandle(table.id, id)} className='add-button'>Save</button>
                                    </div>
                                }
                              </div>
                              <span className='popup__title comment'>Comments</span>
                              <button onClick={() => deleteCard(table.id, id)} className='add-button delete'>Delete card</button>
                            </div>
                          </div>
                          : ''}
